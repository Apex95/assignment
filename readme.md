...
This **decreases the problem's complexity** (the model doesn't have to also learn to isolate entities from a "flat" & noisy text input) and in turn *should* permit **better performance** by delegating the segmentation task to the **web crawler**. At the same time, it is **more favorable** to **annotate datasets** for a simple classifier. Another advantage of this method is the **reduced sequence length**, which avoids a semnificative limitation of the transformer's  attention model; i.e., the NER input is expected to be larger than a possible product name - this is forcing the transformer multi-head attention to use more memory resources or even require input truncation.
...
There should be about **281072** annotated sequences, divided using the **80/20** rule as:
- **training samples:** 234463
- **validation samples:** 46609

The datasets are stored in a **JSON** format; for example, a **positive sample** followed by a **negative sample**:
```json
...
**Dataset imbalance** (or **precision-recall** tradeoff) is tweaked by modifying the penalty of a positive sample at training time.
...
For numeric values: [furniture transformer stats](https://docs.google.com/spreadsheets/d/1iTmqe3nxbfkBlEJHDKh9jAyaIOdPzeN2KpzP8Z7HJlw/edit?usp=sharing)
...
The plot below tracks the possible increase in variance (overfitting) of the model on the training / validation set
